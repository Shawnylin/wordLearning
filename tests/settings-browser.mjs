...
await page.locator('#speech-model-select').selectOption({label:'小米 MiMo · mimo-v2-tts'})
...
const speechPanelFits=await page.locator('.settings-voice-panel').evaluate(el=>el.scrollWidth<=el.clientWidth+1)
assert.equal(speechPanelFits,true)
...
await page.getByText('¥18.50',{exact:true}).waitFor(); await page.getByText('API 余额',{exact:true}).waitFor()
await page.screenshot({path:'docs/.local/profile-mobile.png',fullPage:true})
assert.deepEqual(errors,[])
console.log(JSON.stringify({passed:['button-origin morph','interrupted close','reduced motion','provider save without model selection','PDF and speech share provider','speech controls fit panel','provider balance display','mobile overflow'],errors}))
await browser.close()
